...
- Manifests: `Cargo.toml`, `tauri.conf.json`, `capabilities/` (under src-tauri). The config JSON schema is `schemas/config.v3.schema.json` at the repo ROOT, bundled into `config.rs` via `include_str!("../schemas/...")`.
