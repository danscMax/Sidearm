...
/// One-shot keyboard capture: installs a temporary WH_KEYBOARD_LL hook,
/// waits for the first non-modifier key press, formats it as "Ctrl+Alt+F17",
/// and returns. Used for signal detection where Chromium cannot see real VK
/// codes from Razer Synapse SendInput (F13-F24 appear as "Unidentified").
#[tauri::command]
async fn capture_next_keystroke() -> Result<String, CommandError> {
    tauri::async_runtime::spawn_blocking(capture_one_keystroke_ll)
        .await
        .map_err(|e| CommandError::internal(format!("capture_next_keystroke failed: {e}")))?
}

#[cfg(target_os = "windows")]
fn capture_one_keystroke_ll() -> Result<String, CommandError> {
    use std::cell::Cell;
    ...
    // VK constants for modifier state queries (after capture)
    ...
    // Thread-local storage for the captured VK code
    thread_local! {
        static CAPTURED_VK: Cell<u32> = const { Cell::new(0) };
    }
    ...
    unsafe extern "system" fn hook_proc(code: i32, wparam: usize, lparam: isize) -> isize {
        ...
        // VK constants repeated inside the callback (cannot reference outer consts
        // from an extern "system" fn without statics)
        const VK_SHIFT: u32 = 0x10;
        const VK_CONTROL: u32 = 0x11;
        const VK_MENU: u32 = 0x12;
    ...
    // Reset thread-local before starting
    CAPTURED_VK.with(|c| c.set(0));

    let hmod = unsafe { GetModuleHandleW(std::ptr::null()) };
    let hook = unsafe { SetWindowsHookExW(WH_KEYBOARD_LL, Some(hook_proc), hmod, 0) };
    if hook.is_null() {
    ...
    let vk = CAPTURED_VK.with(|c| c.get());
    if vk == 0 {
        return Err(CommandError::internal("No keystroke captured."));
    }

    // Read modifier state — GetKeyState is valid here because the LL hook
    // fired synchronously on this thread's message pump
    let ctrl = unsafe { GetKeyState(VK_CONTROL as i32) } < 0;
    let alt = unsafe { GetKeyState(VK_MENU as i32) } < 0;
    let shift = unsafe { GetKeyState(VK_SHIFT as i32) } < 0;
    let win = unsafe { GetKeyState(VK_LWIN as i32) } < 0
        || unsafe { GetKeyState(VK_RWIN as i32) } < 0;

    // Format key name using hotkeys::parse_primary_key for consistency with
    // the rest of the codebase, falling back to a VK-based name
    let key_name = vk_to_display_name(vk as u16);

    let mut parts = Vec::new();
    if ctrl { parts.push("Ctrl".to_owned()); }
    if alt { parts.push("Alt".to_owned()); }
    if shift { parts.push("Shift".to_owned()); }
    if win { parts.push("Win".to_owned()); }
    parts.push(key_name);

    Ok(parts.join("+"))
}

/// Map a virtual key code to a human-readable display name.
/// Uses the same naming conventions as `hotkeys::parse_primary_key` for
/// consistency across the codebase.
#[cfg(target_os = "windows")]
fn vk_to_display_name(vk: u16) -> String {
    // F1-F24
    if vk >= 0x70 && vk <= 0x87 {
        let name = format!("F{}", vk - 0x70 + 1);
        // Validate through parse_primary_key for consistency
        if let Ok(key) = parse_primary_key(&name) {
    ...
    // A-Z
    if vk >= 0x41 && vk <= 0x5A {
        let ch = (vk as u8 as char).to_ascii_uppercase().to_string();
        if let Ok(key) = parse_primary_key(&ch) {
            return key.display_name;
        }
        return ch;
    }
    // 0-9
    if vk >= 0x30 && vk <= 0x39 {
        return String::from(vk as u8 as char);
    }
    // Numpad 0-9
    if vk >= 0x60 && vk <= 0x69 {
        return format!("Num{}", vk - 0x60);
    }
    // Named keys — order matches hotkeys.rs display names
    match vk {
        0x08 => "Backspace".into(),
        0x09 => "Tab".into(),
        0x0D => "Enter".into(),
...
fn capture_one_keystroke_ll() -> Result<String, CommandError> {
    Err(CommandError::internal(
        "Keystroke capture is only available on Windows.",
    ))
}
